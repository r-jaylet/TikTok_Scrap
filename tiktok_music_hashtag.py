...
def tiktok(only_unverified=True,
    ...
    hashtag_list = hashtag_start.split()

    for hashtag in hashtag_list:

        start = timer()
        tag = api.hashtag(name=hashtag)
        tiktoks = [video.as_dict for video in tag.videos()]  # count=n_tiktok
        end = timer()
        print("Temps d'exécution de la recherche pour", hashtag, ":", round(end - start, 1), 's')
